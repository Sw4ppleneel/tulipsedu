    ...
    <section id="admissions" style={{ background: `linear-gradient(135deg, ${C.navy} 0%, ${C.maroon} 100%)` }}>
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '4.5rem 1.25rem' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <a href="tel:+91XXXXXXXXXX" style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', background: C.gold, color: C.navy, borderRadius: 7, padding: '.9rem 2.3rem', fontFamily: FONT, fontWeight: 800, fontSize: '1rem', textDecoration: 'none', transition: 'background .2s' }}
            onMouseOver={e => { (e.currentTarget as HTMLElement).style.background = C.goldLt }} onMouseOut={e => { (e.currentTarget as HTMLElement).style.background = C.gold }}>
            Enquire About Admissions <Ico name="arrowRight" size={17} stroke={C.navy} width={2} />
          </a>
          <p style={{ color: 'rgba(255,255,255,.5)', fontSize: '.78rem', marginTop: '.8rem' }}>Call us · Visit the school office · Monday to Saturday, 9 AM to 4 PM</p>
        </div>
      </div>
    </section>
  )
