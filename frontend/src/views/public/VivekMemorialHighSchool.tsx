...
// ── Palette — warm "Vivek Memorial" identity: golden yellow + cream + espresso ──
const C = {
  gold:    '#D4A017',
  goldLt:  '#F2C94C',
  goldDk:  '#A97712',
...
// Serif stack — a classic, warm letterhead feel using system fonts only.
...
function IconChip({ name, size = 52 }: { name: IcoName; size?: number }) {
  return (
    <div style={{
      ...
      background: `${C.tan}88`, display: 'flex', alignItems: 'center', justifyContent: 'center',
      color: C.brown,
    }}>
...
function ImgPlaceholder({ label, ratio = 56, radius = 0, style = {} }: { label: string; ratio?: number; radius?: number; style?: preact.JSX.CSSProperties }) {
  return (
    <div style={{ position: 'relative', paddingBottom: `${ratio}%`, background: C.creamDk, borderRadius: radius, overflow: 'hidden', ...style }}>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: C.tanDk, gap: 8 }}>
...
function SchoolLogo({ slug, name, size = 46 }: { slug: string; name: string; size?: number }) {
  const candidates = ['logo.svg?v=1', 'logo.png?v=1']
  const [idx, setIdx] = useState(0)
  if (!slug || idx >= candidates.length) {
    return (
      <div style={{ width: size, height: size, borderRadius: '50%', background: `linear-gradient(135deg, ${C.goldLt}, ${C.goldDk})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.inkDk, fontWeight: 900, fontSize: size * 0.42, flexShrink: 0 }}>
...
function SectionHead({ label, title, sub, dark = false }: { label: string; title: string; sub?: string; dark?: boolean }) {
  return (
    <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
      <div style={{ display: 'inline-block', fontSize: '.7rem', fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: C.goldDk, marginBottom: '.6rem' }}>{label}</div>
      <h2 style={{ fontSize: 'clamp(1.5rem,3vw,2.25rem)', fontWeight: 800, color: dark ? C.white : C.ink, margin: '0 0 .85rem', lineHeight: 1.2 }}>{title}</h2>
      <div style={{ width: 54, height: 3, borderRadius: 2, background: C.gold, margin: '0 auto' }} />
      {sub && <p style={{ color: dark ? 'rgba(255,255,255,.78)' : C.gray600, maxWidth: 580, margin: '1.1rem auto 0', lineHeight: 1.7, fontSize: '.95rem' }}>{sub}</p>}
    </div>
  )
}
...
function Navbar({ slug, name, onStaffLogin, onParentLogin }: { slug: string; name: string; onStaffLogin: () => void; onParentLogin: () => void }) {
  ...
  return (
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000, fontFamily: FONT, transition: 'background .3s, box-shadow .3s', background: scrolled ? C.inkDk : 'transparent', boxShadow: scrolled ? '0 2px 24px rgba(0,0,0,.35)' : 'none' }}>
      {/* Utility bar */}
      <div style={{ background: C.inkDk, borderBottom: '1px solid rgba(212,160,23,.22)', padding: '.32rem 1.25rem' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '.3rem' }}>
          <div style={{ display: 'flex', gap: '1.4rem', fontSize: '.72rem', color: 'rgba(255,255,255,.7)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '.35rem' }}><Ico name="phone" size={12} stroke={C.goldLt} /> 0651-2360145</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '.35rem' }}><Ico name="mapPin" size={12} stroke={C.goldLt} /> Doranda — Ranchi</span>
          </div>
          <div style={{ display: 'flex', gap: '.6rem' }}>
            <button onClick={onParentLogin} style={{ background: 'none', border: `1px solid ${C.gold}`, color: C.goldLt, borderRadius: 5, padding: '.22rem .7rem', cursor: 'pointer', fontSize: '.7rem', fontFamily: FONT, fontWeight: 600 }}>Parent Login</button>
            <button onClick={onStaffLogin} style={{ background: C.gold, border: 'none', color: C.inkDk, borderRadius: 5, padding: '.22rem .7rem', cursor: 'pointer', fontSize: '.7rem', fontFamily: FONT, fontWeight: 700 }}>Staff Login</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '.7rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', cursor: 'pointer' }} onClick={() => scrollTo('home')}>
          <SchoolLogo slug={slug} name={name} size={46} />
          <div>
            <div style={{ color: C.white, fontWeight: 800, fontSize: '1rem', lineHeight: 1.1, letterSpacing: '-.01em' }}>Vivek Memorial High School</div>
            <div style={{ color: C.goldLt, fontSize: '.64rem', fontWeight: 600, letterSpacing: '.06em' }}>DORANDA, RANCHI · ESTD. {ESTD}</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '1.75rem', alignItems: 'center' }} class="vmhs-desk-nav">
          {NAV_LINKS.map(l => (
            <button key={l} onClick={() => scrollTo(l.toLowerCase())} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,.85)', cursor: 'pointer', fontFamily: FONT, fontSize: '.82rem', fontWeight: 600, padding: 0, transition: 'color .2s' }}
              onMouseOver={e => (e.currentTarget.style.color = C.goldLt)} onMouseOut={e => (e.currentTarget.style.color = 'rgba(255,255,255,.85)')}>{l}</button>
          ))}
        </div>

        <button onClick={() => setMenuOpen(o => !o)} style={{ display: 'none', background: 'none', border: 'none', cursor: 'pointer', color: C.white, padding: '.25rem' }} class="vmhs-hamburger">
      ...
      {menuOpen && (
        <div style={{ background: C.inkDk, borderTop: '1px solid rgba(255,255,255,.08)', padding: '1rem 1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
          {NAV_LINKS.map(l => (
      ...
      <style>{`@media (max-width: 820px){.vmhs-desk-nav{display:none!important}.vmhs-hamburger{display:block!important}}`}</style>
    </nav>
  )
}
...
function Hero({ slug, schoolName, announcements }: { slug: string; schoolName: string; announcements: CmsAnnouncement[] }) {
  ...
  const years = new Date().getFullYear() - ESTD

  return (
    <div id="home" style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', background: C.inkDk, fontFamily: FONT, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, zIndex: 0, background: 'linear-gradient(155deg, #3A2A0E 0%, #5B431A 52%, #4A3712 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 10, color: 'rgba(255,255,255,.08)' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: 'linear-gradient(to right, rgba(58,42,14,.94) 48%, rgba(58,42,14,.55) 100%)' }} />
      ...
      <div style={{ position: 'relative', zIndex: 2, flex: 1, display: 'flex', alignItems: 'center', maxWidth: 1120, margin: '0 auto', padding: '8rem 1.25rem 4rem', width: '100%', boxSizing: 'border-box' }}>
        <div style={{ maxWidth: 640 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', background: 'rgba(212,160,23,.16)', border: '1px solid rgba(212,160,23,.34)', borderRadius: 9999, padding: '.35rem 1rem', marginBottom: '1.5rem', color: C.goldLt, fontSize: '.74rem', fontWeight: 700, letterSpacing: '.08em' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.goldLt, display: 'inline-block' }} />
            ESTD. {ESTD} · JHARKHAND ACADEMIC COUNCIL
          </div>
          ...
          <h1 style={{ color: C.white, fontSize: 'clamp(2.1rem,5vw,3.6rem)', fontWeight: 900, lineHeight: 1.08, margin: '0 0 .5rem', letterSpacing: '-.02em' }}>
            {schoolName || 'Vivek Memorial High School'}
          </h1>
          <div style={{ color: C.goldLt, fontSize: 'clamp(1rem,2vw,1.3rem)', fontWeight: 600, marginBottom: '1.25rem' }}>Doranda — Ranchi, Jharkhand</div>
          <p style={{ color: 'rgba(255,255,255,.78)', fontSize: '1.05rem', lineHeight: 1.7, margin: '0 0 1.5rem', maxWidth: 510 }}>
            A neighbourhood school guiding students from the early grades to Class X (Matric) under the Jharkhand Academic Council — built on the values of knowledge, character, and service.
          </p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.6rem', marginBottom: '2.25rem', color: C.tan, fontStyle: 'italic', fontSize: '.95rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            <button onClick={() => scrollTo('admissions')} style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', background: C.gold, color: C.inkDk, border: 'none', borderRadius: 7, padding: '.85rem 1.9rem', fontFamily: FONT, fontWeight: 800, fontSize: '1rem', cursor: 'pointer', transition: 'transform .15s, background .15s' }}
              ...
              Apply for Admissions <Ico name="arrowRight" size={17} stroke={C.inkDk} width={2} />
            </button>
            <button onClick={() => scrollTo('about')} style={{ background: 'transparent', color: C.white, border: '2px solid rgba(255,255,255,.32)', borderRadius: 7, padding: '.85rem 1.9rem', fontFamily: FONT, fontWeight: 700, fontSize: '1rem', cursor: 'pointer', transition: 'border-color .15s, color .15s' }}
              onMouseOver={e => { e.currentTarget.style.borderColor = C.goldLt; e.currentTarget.style.color = C.goldLt }}
      ...
      <div style={{ position: 'relative', zIndex: 2, borderTop: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.35)', backdropFilter: 'blur(8px)' }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', padding: '1.25rem', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem' }}>
          {[
            { num: `${ESTD}`, label: 'Established' },
            { num: 'I–X', label: 'Classes Offered' },
            { num: `${years}+`, label: 'Years of Service' },
            { num: 'Matric', label: 'JAC Board' },
          ].map(s => (
            <div key={s.label} style={{ textAlign: 'center' }}>
              <div style={{ color: C.goldLt, fontSize: 'clamp(1.1rem,3vw,1.8rem)', fontWeight: 900, lineHeight: 1 }}>{s.num}</div>
      ...
      {announcements.length > 0 && (
        <div style={{ position: 'relative', zIndex: 2, background: C.gold, padding: '.5rem 1.25rem', display: 'flex', gap: '.75rem', alignItems: 'center', overflow: 'hidden' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', color: C.inkDk, fontWeight: 800, fontSize: '.74rem', whiteSpace: 'nowrap', flexShrink: 0, letterSpacing: '.04em' }}>
            <Ico name="megaphone" size={15} stroke={C.inkDk} /> NOTICE
          </span>
          <div style={{ overflow: 'hidden', flex: 1 }}>
            <div style={{ whiteSpace: 'nowrap', animation: 'vmhs-ticker 30s linear infinite', display: 'inline-block', color: C.inkDk, fontSize: '.8rem', fontWeight: 600 }}>
              {announcements.map(a => a.title).join('     •     ')}
            </div>
          </div>
          <style>{`@keyframes vmhs-ticker{from{transform:translateX(100%)}to{transform:translateX(-100%)}}`}</style>
        </div>
      )}
    </div>
  )
...
function About({ slug }: { slug: string }) {
  const cards: { icon: IcoName; title: string; body: string }[] = [
    { icon: 'target', title: 'Our Mission', body: 'To offer affordable, value-based education that prepares every student for the Class X (Matric) board examination and for a confident, capable next step in life.' },
    { icon: 'star', title: 'Our Vision', body: 'To be a trusted school in Doranda and greater Ranchi — nurturing knowledge, discipline, and good character from the early grades through Class X.' },
    { icon: 'heart', title: 'Our Values', body: 'Honesty, regularity, and respect guide everything we do. We treat every child as our own and help each one grow steadily, year after year.' },
  ]
  return (
    <Section id="about" bg={C.cream}>
      <SectionHead label="About the School" title={`Serving Ranchi Since ${ESTD}`} sub="Vivek Memorial High School offers schooling up to Class X (Matric) under the Jharkhand Academic Council, with a steady record of service to families in Doranda and across Ranchi." />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(290px,1fr))', gap: '2rem', alignItems: 'stretch' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {cards.map(c => (
            <div key={c.title} style={{ background: C.white, borderRadius: 12, padding: '1.25rem', border: `1px solid ${C.creamDk}`, boxShadow: '0 2px 10px rgba(91,67,26,.06)', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
              <IconChip name={c.icon} size={46} />
              <div>
                <div style={{ fontWeight: 700, color: C.ink, marginBottom: '.3rem', fontSize: '.95rem' }}>{c.title}</div>
...
function Stats() {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg, ${C.brown}, ${C.inkDk})`, fontFamily: FONT }}>
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '3rem 1.25rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '1.5rem' }}>
        {items.map((s, i) => (
          <div key={s.label} style={{ textAlign: 'center', padding: '1.25rem', borderRight: i < items.length - 1 ? '1px solid rgba(255,255,255,.1)' : 'none' }}>
            <div style={{ fontSize: 'clamp(2rem,4vw,2.8rem)', fontWeight: 900, color: C.goldLt, lineHeight: 1 }}>{s.num}</div>
...
function Facilities() {
  const items: { icon: IcoName; title: string; body: string }[] = [
    ...
    { icon: 'users', title: 'Experienced Teachers', body: 'Dedicated teachers who know the JAC syllabus and guide students towards strong board results.' },
  ...
  return (
    <Section id="facilities" bg={C.white}>
      <SectionHead label="Why Choose Us" title="Everything a Good School Should Be" sub="Steady teaching combined with the facilities students need to do well in their Class X board examination." />
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', justifyContent: 'center' }}>
        {items.map(i => (
          <div key={i.title} style={{ flex: '1 1 300px', maxWidth: 340, background: C.cream, borderRadius: 14, padding: '1.6rem', border: `1px solid ${C.creamDk}`, transition: 'transform .2s, box-shadow .2s' }}
            onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 10px 28px rgba(91,67,26,.14)' }}
            onMouseOut={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '' }}>
            <IconChip name={i.icon} size={54} />
            <div style={{ fontWeight: 700, color: C.ink, margin: '1rem 0 .4rem', fontSize: '1.05rem' }}>{i.title}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: '3rem', alignItems: 'center' }}>
        <div style={{ maxWidth: 320, margin: '0 auto', width: '100%' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '1rem' }}>
            <div style={{ fontWeight: 700, color: C.ink, fontSize: '1rem' }}>[Principal's Name]</div>
            <div style={{ color: C.brown, fontSize: '.8rem', fontWeight: 600 }}>Principal, Vivek Memorial High School</div>
          </div>
        </div>
        <div>
          ...
          <p style={{ color: C.gray600, fontSize: '1.02rem', lineHeight: 1.8, margin: '0 0 1rem', fontStyle: 'italic' }}>
            At Vivek Memorial High School, our promise is simple: to teach with sincerity, treat every child with care, and prepare each student well for the Class X board and the road beyond.
          </p>
          <p style={{ color: C.gray600, fontSize: '1.02rem', lineHeight: 1.8, margin: '0 0 1.5rem', fontStyle: 'italic' }}>
            With committed teachers and a disciplined, friendly campus, we help every student grow in knowledge, confidence, and good values.
          </p>
          <div style={{ display: 'flex', gap: '1rem' }}>
            <div style={{ width: 4, background: C.gold, borderRadius: 2, flexShrink: 0 }} />
            <div>
              <div style={{ fontWeight: 700, color: C.ink }}>Warm regards,</div>
...
function Academics() {
  const stages: { icon: IcoName; grade: string; range: string; bg: string; bd: string }[] = [
    { icon: 'sprout', grade: 'Primary', range: 'Class I — V', bg: '#FDF6E0', bd: '#EFDDA6' },
    { icon: 'pencil', grade: 'Middle School', range: 'Class VI — VIII', bg: '#FBF0D6', bd: '#E9CF8F' },
    { icon: 'flask', grade: 'Secondary', range: 'Class IX — X · Matric', bg: '#F4EBCB', bd: '#DCC079' },
  ]
  return (
    <Section id="academics" bg={C.white}>
      <SectionHead label="Academics" title="A Steady Path to Matric" sub="From the primary grades through Class X, Vivek Memorial High School offers a structured learning pathway aligned with the Jharkhand Academic Council syllabus." />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '1.25rem', marginBottom: '2.5rem' }}>
        {stages.map(s => (
          <div key={s.grade} style={{ background: s.bg, border: `1px solid ${s.bd}`, borderRadius: 14, padding: '1.75rem 1.25rem', textAlign: 'center' }}>
            <div style={{ display: 'inline-flex', width: 56, height: 56, borderRadius: 16, background: C.white, alignItems: 'center', justifyContent: 'center', color: C.brown, marginBottom: '.85rem', boxShadow: '0 2px 8px rgba(0,0,0,.05)' }}>
              <Ico name={s.icon} size={28} width={1.7} />
            </div>
            <div style={{ fontWeight: 700, color: C.ink, fontSize: '1rem', marginBottom: '.3rem' }}>{s.grade}</div>
            <div style={{ color: C.gray600, fontSize: '.82rem' }}>{s.range}</div>
          </div>
        ))}
      </div>
      <div style={{ background: C.ink, borderRadius: 14, padding: '1.5rem 2rem', display: 'flex', alignItems: 'center', gap: '1.25rem', flexWrap: 'wrap' }}>
        <div style={{ color: C.goldLt }}><Ico name="cap" size={34} stroke={C.goldLt} /></div>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ color: C.white, fontWeight: 700, marginBottom: '.25rem' }}>Secondary Education · Jharkhand Academic Council</div>
          <div style={{ color: 'rgba(255,255,255,.7)', fontSize: '.82rem', lineHeight: 1.6 }}>Vivek Memorial High School follows the JAC curriculum up to Class X, with a strong focus on the core subjects and steady preparation for the Matriculation board examination.</div>
        </div>
      </div>
    </Section>
  )
...
function Gallery({ slug }: { slug: string }) {
  ...
  return (
    <Section id="gallery" bg={C.cream}>
      <SectionHead label="Gallery" title="Life at Vivek Memorial" sub="A glimpse into our campus and community — from classrooms and the lab to the playground and school events." />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(230px,1fr))', gap: '.85rem' }}>
        {photos.map((p, i) => (
          <div key={i} style={{ overflow: 'hidden', borderRadius: 12, cursor: 'pointer', transition: 'transform .2s, box-shadow .2s' }}
            onMouseOver={e => { e.currentTarget.style.transform = 'scale(1.02)'; e.currentTarget.style.boxShadow = '0 8px 22px rgba(91,67,26,.18)' }}
            onMouseOut={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '' }}>
            ...
            <div key={a.id} style={{ display: 'flex', gap: '1.25rem', alignItems: 'flex-start', background: C.cream, borderRadius: 14, padding: '1.25rem 1.5rem', border: `1px solid ${C.creamDk}`, borderLeft: `4px solid ${C.gold}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1rem', flexWrap: 'wrap' }}>
                  <div style={{ fontWeight: 700, color: C.ink, fontSize: '1rem' }}>{a.title}</div>
                  {a.published_at && (
                ...
                {a.expires_at && (
                  <div style={{ marginTop: '.5rem', fontSize: '.72rem', color: C.brown, fontWeight: 600 }}>Valid until {fmtDate(a.expires_at)}</div>
                )}
              </div>
            </div>
          ))}
...
function Admissions() {
  ...
  return (
    <section id="admissions" style={{ background: `linear-gradient(135deg, ${C.brown} 0%, ${C.inkDk} 100%)` }}>
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '4.5rem 1.25rem' }}>
        <SectionHead label="Admissions Open" title="Join Vivek Memorial High School" sub="We welcome applications up to Class X, subject to seat availability. Admissions for the new academic session are now open at the school office." dark />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
          {steps.map(s => (
            <div key={s.n} style={{ background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.13)', borderRadius: 14, padding: '1.6rem' }}>
              <div style={{ color: C.goldLt, fontWeight: 900, fontSize: '1.6rem', marginBottom: '.5rem' }}>{s.n}</div>
              <div style={{ color: C.white, fontWeight: 700, marginBottom: '.4rem', fontSize: '.98rem' }}>{s.title}</div>
              <p style={{ color: 'rgba(255,255,255,.66)', fontSize: '.83rem', lineHeight: 1.65, margin: 0 }}>{s.body}</p>
            </div>
          ))}
        </div>
        <AdmissionForm theme={{ accent: C.gold, accentText: C.inkDk, font: FONT }} />
        <p style={{ color: 'rgba(255,255,255,.55)', fontSize: '.78rem', marginTop: '1rem', textAlign: 'center' }}>Prefer to call? 0651-2360145 · Visit the school office · Monday to Saturday, 9 AM to 3 PM</p>
      </div>
    </section>
  )
}
...
function Contact() {
  const info: { icon: IcoName; label: string; lines: string[] }[] = [
    { icon: 'mapPin', label: 'Address', lines: ['Vivek Memorial High School', 'Doranda', 'Ranchi — 834002, Jharkhand'] },
    { icon: 'phone', label: 'Phone', lines: ['0651-2360145', 'Mob: 98044-44444'] },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {info.map(i => (
            <div key={i.label} style={{ background: C.cream, borderRadius: 12, padding: '1.2rem', border: `1px solid ${C.creamDk}` }}>
              ...
              <div style={{ fontWeight: 700, color: C.ink, fontSize: '.82rem', margin: '.6rem 0 .4rem' }}>{i.label}</div>
              {i.lines.map(l => <div key={l} style={{ color: C.gray600, fontSize: '.8rem', lineHeight: 1.6 }}>{l}</div>)}
            </div>
          ))}
        </div>
        <div>
          <ImgPlaceholder label="Google Maps — Vivek Memorial High School, Doranda Ranchi" ratio={72} radius={14} />
          <a href="https://maps.google.com/?q=Doranda+Ranchi+Jharkhand" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '.35rem', marginTop: '.85rem', color: C.ink, fontWeight: 600, fontSize: '.82rem', textDecoration: 'none' }}>
            Open in Google Maps <Ico name="externalLink" size={14} stroke={C.ink} />
          </a>
        </div>
      </div>
    </Section>
...
function Footer({ slug, name, onStaffLogin, onParentLogin }: { slug: string; name: string; onStaffLogin: () => void; onParentLogin: () => void }) {
  ...
  return (
    <footer style={{ background: C.inkDk, color: 'rgba(255,255,255,.72)', fontFamily: FONT }}>
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '3.25rem 1.25rem 1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '2.5rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', marginBottom: '1rem' }}>
            <SchoolLogo slug={slug} name={name} size={40} />
            <div>
              <div style={{ color: C.white, fontWeight: 800, fontSize: '.88rem' }}>Vivek Memorial High School</div>
              <div style={{ color: C.goldLt, fontSize: '.62rem', fontWeight: 600, letterSpacing: '.05em' }}>DORANDA, RANCHI · ESTD. {ESTD}</div>
            </div>
          </div>
          <p style={{ fontSize: '.8rem', lineHeight: 1.7, margin: 0, maxWidth: 240 }}>A Jharkhand Academic Council school offering education up to Class X (Matric) in Doranda, Ranchi — built on knowledge, character, and service.</p>
        </div>

        <div>
          <div style={{ color: C.goldLt, fontWeight: 700, fontSize: '.78rem', letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: '1rem' }}>Quick Links</div>
          {NAV_LINKS.slice(1).map(l => (
            <div key={l} style={{ marginBottom: '.45rem' }}>
              <a href={`#${l.toLowerCase()}`} style={{ color: 'rgba(255,255,255,.68)', fontSize: '.82rem', textDecoration: 'none' }}
                onMouseOver={e => { (e.currentTarget as HTMLElement).style.color = C.goldLt }} onMouseOut={e => { (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,.68)' }}>{l}</a>
            </div>
          ))}
        </div>

        <div>
          <div style={{ color: C.goldLt, fontWeight: 700, fontSize: '.78rem', letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: '1rem' }}>Contact</div>
          <p style={{ fontSize: '.82rem', lineHeight: 1.85, margin: 0 }}>
            Doranda<br />Ranchi — 834002, Jharkhand<br />0651-2360145<br />Affiliated to JAC
          </p>
        </div>

        <div>
          <div style={{ color: C.goldLt, fontWeight: 700, fontSize: '.78rem', letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: '1rem' }}>Portal Access</div>
          <button onClick={onParentLogin} style={{ display: 'flex', alignItems: 'center', gap: '.55rem', background: 'rgba(212,160,23,.16)', border: '1px solid rgba(212,160,23,.34)', color: C.goldLt, borderRadius: 7, padding: '.55rem 1rem', fontFamily: FONT, fontWeight: 600, fontSize: '.82rem', cursor: 'pointer', width: '100%', marginBottom: '.55rem' }}>
            <Ico name="users" size={16} stroke={C.goldLt} /> Parent Portal
          </button>
          <button onClick={onStaffLogin} style={{ display: 'flex', alignItems: 'center', gap: '.55rem', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)', color: 'rgba(255,255,255,.8)', borderRadius: 7, padding: '.55rem 1rem', fontFamily: FONT, fontWeight: 600, fontSize: '.82rem', cursor: 'pointer', width: '100%' }}>
            <Ico name="building" size={16} stroke="rgba(255,255,255,.8)" /> Staff Portal
          </button>
        </div>
      </div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', maxWidth: 1120, margin: '0 auto', padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '.5rem' }}>
        <span style={{ fontSize: '.72rem' }}>© {year} Vivek Memorial High School, Doranda Ranchi. All rights reserved.</span>
...
// ── Root ──────────────────────────────────────────────────────────────────────
export function VivekMemorialHighSchool({ onStaffLogin, onParentLogin }: { onStaffLogin: () => void; onParentLogin: () => void }) {
