...
async def main():
    ...
    async with conn.transaction():
        ...
        # set_student_discounts's recompute. Correlated subquery in SET (not a JOIN
        # in FROM) since Postgres UPDATE...FROM can't reference the target table (fl)
        # inside a FROM-clause JOIN's ON condition.
        result = await conn.execute(
            """
            UPDATE fee_ledger fl
            SET amount_due = ROUND(fs.amount * $4 / 100 * (100 - COALESCE(
                (SELECT d.percentage FROM student_fee_discounts d
                 WHERE d.tenant_id = fl.tenant_id AND d.student_id = fl.student_id
                   AND d.fee_head_id = fs.fee_head_id),
                0
            )) / 100, 2)
